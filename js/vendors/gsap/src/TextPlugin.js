...
let gsap, _tempDiv, _getGSAP = () => gsap || (typeof (window) !== "undefined" && (gsap = window.gsap) && gsap.registerPlugin && gsap);
export const TextPlugin = {
    version: "3.5.1",
    name: "text",
    init(target, value, tween) {
        let i = target.nodeName.toUpperCase(), data = this, short, text, original, j, condensedText, condensedOriginal, aggregate, s;
    ...
    render(ratio, data) {
        ...
        let { text, hasClass, newClass, oldClass, delimiter, target, fillChar, original } = data, l = text.length, i = (ratio * l + 0.5) | 0, applyNew, applyOld, str;
        if (hasClass) {
            applyNew = (newClass && i);
