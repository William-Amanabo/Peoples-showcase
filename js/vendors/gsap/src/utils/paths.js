...
export function sliceRawPath(rawPath, start, end) {
    ...
    start = start || 0;
    let reverse = start > end, loops = Math.max(0, ~~(_abs(end - start) - 1e-8));
    if (reverse) {
        reverse = end;
        end = start;
        start = reverse;
...
export function stringToRawPath(d) {
    ...
    path = [], relativeX = 0, relativeY = 0, twoThirds = 2 / 3, elements = a.length, points = 0, errorMessage = "ERROR: malformed path: " + d, i, j, x, y, command, isRelative, segment, startX, startY, difX, difY, beziers, prevCommand, flag1, flag2, line = function (sx, sy, ex, ey) {
        difX = (ex - sx) / 3;
        difY = (ey - sy) / 3;
...
export function pointsToSegment(points, curviness, cornerThreshold) {
    ...
    let l = points.length - 2, x = +points[0], y = +points[1], nextX = +points[2], nextY = +points[3], segment = [x, y, x, y], dx2 = nextX - x, dy2 = nextY - y, closed = Math.abs(points[l] - x) < 0.001 && Math.abs(points[l + 1] - y) < 0.001, prevX, prevY, angle, slope, i, dx1, dx3, dy1, dy3, d1, d2, a, b, c;
...
export function simplifyPoints(points, tolerance) {
    let prevX = parseFloat(points[0]), prevY = parseFloat(points[1]), temp = [prevX, prevY], l = points.length - 2, i, x, y, dx, dy, result, last;
...
export function getClosestData(rawPath, x, y, slices) {
    let closest = { j: 0, i: 0, t: 0 }, bestDistance = _largeNum, i, j, t, segment;
