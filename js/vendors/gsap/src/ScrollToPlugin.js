...
*/
/* eslint-disable */
let gsap, _coreInitted, _window, _docEl, _body, _toArray, _config, _windowExists = () => typeof (window) !== "undefined", _getGSAP = () => gsap || (_windowExists() && (gsap = window.gsap) && gsap.registerPlugin && gsap), _isString = value => typeof (value) === "string", _max = (element, axis) => {
    ...
    let rect = _toArray(element)[0].getBoundingClientRect(), isRoot = (!container || container === _window || container === _body), cRect = isRoot ? { top: _docEl.clientTop - (_window.pageYOffset || _docEl.scrollTop || _body.scrollTop || 0), left: _docEl.clientLeft - (_window.pageXOffset || _docEl.scrollLeft || _body.scrollLeft || 0) } : container.getBoundingClientRect(), offsets = { x: rect.left - cRect.left, y: rect.top - cRect.top };
...
export const ScrollToPlugin = {
    ...
    render(ratio, data) {
        let pt = data._pt, { target, tween, autoKill, xPrev, yPrev, isWin } = data, x, y, yDif, xDif, threshold;
        while (pt) {
            pt.r(ratio, pt.d);
            pt = pt._next;
        }
