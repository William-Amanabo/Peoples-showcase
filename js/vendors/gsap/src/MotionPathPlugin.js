...
let _xProps = ["x", "translateX", "left", "marginLeft"], _yProps = ["y", "translateY", "top", "marginTop"], _DEG2RAD = Math.PI / 180, gsap, PropTween, _getUnit, _toArray, _getGSAP = () => gsap || (typeof (window) !== "undefined" && (gsap = window.gsap) && gsap.registerPlugin && gsap), _populateSegmentFromArray = (segment, values, property, mode) => {
    let l = values.length, si = mode === 2 ? 0 : mode, i = 0;
    for (; i < l; i++) {
...
}, _emptyFunc = v => v, _numExp = /[-+\.]*\d+[\.e\-\+]*\d*[e\-\+]*\d*/g, _originToPoint = (element, origin, parentMatrix) => {
    let m = getGlobalMatrix(element), svg, x, y;
    ...
    let parentMatrix = getGlobalMatrix(fromElement.parentNode, true, true), m = parentMatrix.clone().multiply(getGlobalMatrix(toElement)), fromPoint = _originToPoint(fromElement, fromOrigin, parentMatrix), { x, y } = _originToPoint(toElement, toOrigin, parentMatrix), p;
    m.e = m.f = 0;
...
export const MotionPathPlugin = {
    ...
    init(target, vars) {
        ...
        let rawPaths = [], path = vars.path, firstObj = path[0], autoRotate = vars.autoRotate, slicer = _sliceModifier(vars.start, ("end" in vars) ? vars.end : 1), rawPath, p, x, y;
